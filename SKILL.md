...
1. `Pre-manuscript intake.` Read the current project folder, identify available data/code/templates/results, ask or infer the target journal, clarify the decision problem, choose the model family, and choose the solution algorithm family.
...
Do not skip directly to prose drafting when the task is a new full paper. If the user has not supplied enough information, ask concise intake questions first; if the user asks to proceed, make documented assumptions and continue.
...
Ask these intake questions concisely, preferably in one short grouped message. Once enough information is available, produce a contribution ledger and manuscript plan before writing long prose.
## Core OR Spine
...
When benchmark data have actually been downloaded and benchmark experiments have been run, replace planning language such as `should be used`, `should be replaced`, or `recommended benchmark design` in the main computational-study text with executed-result language. State where the public instances came from, how many were downloaded or extracted, which subset was selected for the reported short test, what script generated the results, and what evidence boundary remains. Placeholder benchmark tables may remain only in appendices or work notes; the main text should report traceable generated tables and figures.

Do not leave blank cells, dashes, or placeholder rows in main-text result tables for an experiment that supports a contribution. If the experiment has not been run, either run a traceable prototype, move the design template to an appendix as a planned validation protocol, or remove the table from the main text. When using a prototype rather than a final calibrated model, label the table as prototype evidence and state the boundary directly in the surrounding prose.
...
When adding extra experiments late in a manuscript, prefer a moderate, diagnostic appendix test over a long unfocused benchmark sweep. Define the subset by a defensible instance feature such as scale, capacity tightness, uncertainty level, or route-competition intensity; state that role explicitly; and report generated output files, seeds or fixed settings, and boundaries. Do not add an appendix table merely because it exists: include it only when it clarifies a contribution, supports an ablation, or explains where the proposed method has an advantage. If a broad scan weakens or blurs the story, summarize the boundary in prose rather than hiding it or forcing it into the paper.
...
For stochastic heuristics, do not base a main superiority claim on a single fixed seed when the reported improvement is marginal. Use at least 10 seeds for the main stochastic comparison whenever runtime allows, report `mean ± sd`, and add paired statistical evidence such as a Wilcoxon signed-rank test plus a paired bootstrap confidence interval. If the confidence interval includes zero or the paired test is not significant, frame the result as competitiveness or boundary evidence, not dominance.
...
- innovation boundary: state precisely what is new in the paper and what is adopted from established methods;
...
- Do not use a hand-built rule proxy, manually assigned score, or placeholder probability as the main evidence for a learning-assisted contribution. Such proxies may appear only as baselines, smoke tests, or preliminary diagnostics, and the boundary must be explicit.
...
- `Main manuscript.` Provide the cleaned manuscript source and compiled PDF using the correct template. Preserve the target journal's template fonts, spacing, front matter, bibliography style, and figure/table conventions unless the user explicitly asks otherwise.
